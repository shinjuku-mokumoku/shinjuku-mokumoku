...
const meetupFiles = async (num) => {
  ...
  const fileNames = res.data.map(files => files.filename)
...
const isValidFormatFile = async (prNumber, filePath) => {
  const resPullRequest = await octokit.pulls.get({ owner, repo, pull_number: prNumber });
  const [headOrg, headBranch] = resPullRequest.data.head.label.split(':');

  const resContent = await octokit.repos.getContents({
    owner: headOrg, repo, path: filePath, ref: headBranch,
  });
  const content = Buffer.from(resContent.data.content, 'base64').toString('UTF-8');
